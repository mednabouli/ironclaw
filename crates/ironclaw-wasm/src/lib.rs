...
impl Tool for WasmTool {
    ...
    async fn invoke(&self, _params: Value) -> anyhow::Result<Value> {
        warn!("WASM plugin '{}' called but wasmtime is not yet wired (Phase 6)", self.schema.name);
...
pub fn scan_plugins(dir: &std::path::Path) -> Vec<WasmTool> {
    ...
    entries
        ...
        .map(|e| {
            ...
            let schema = ToolSchema { name: name.clone(), description: format!("WASM plugin: {name}"), parameters: json!({"type":"object","properties":{}}) };
