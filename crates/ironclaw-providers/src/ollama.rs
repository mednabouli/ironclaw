...
impl Provider for OllamaProvider {
    ...
    async fn complete(&self, req: CompletionRequest) -> anyhow::Result<CompletionResponse> {
        ...
        let resp: Value = self.client
            ...
            .json(&body)
            .send().await.context("Ollama HTTP error")?
            .error_for_status().context("Ollama API error")?
            .json().await.context("Ollama JSON parse")?;
        ...
        if let Some(arr) = resp["message"]["tool_calls"].as_array() {
            for tc in arr {
                ...
                tool_calls.push(ToolCall { id: uuid::Uuid::new_v4().to_string(), name, arguments: args });
            }
        }
        ...
        Ok(CompletionResponse {
            ...
            usage: TokenUsage { prompt_tokens, completion_tokens, total_tokens: prompt_tokens + completion_tokens },
    ...
    async fn stream(&self, req: CompletionRequest) -> anyhow::Result<BoxStream<StreamChunk>> {
        ...
        let response = self.client
            .post(format!("{}/api/chat", self.base_url))
            .json(&body)
            .send().await.context("Ollama stream error")?
            .error_for_status()?;
